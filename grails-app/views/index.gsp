	...
	<div id="container" style="margin-left: 50px">
		<div class="row">
			<div class="col-xs-5">
				<div class="row">
					<h3>Select Cards</h3>
					<button id="clear" type="button" class="btn btn-primary">New
						Hand</button>
					<div class="col-xs-12">
			...
			<div class="col-xs-3" id="selected-cards">
				<div class="row">
					<div id="pocket-header" style="display: none">
						<h3>Pocket Cards</h3>
					</div>
					<div id="pocket-pair">
						<div id="cardA"></div>
		...
		<script>
			$(".card").on("click", function(c) {
				...
				$("#selected-cards .card").each(function(d, c) {
					if ($(c).find("#cardValue").val() == val) {
						...
						.ajax({
							...
							success : function(html) {
								...
								if (target == "#river") {
									...
									jQuery
											.ajax({
												url : '<g:createLink controller="evaluator" action="evaluateHand"/>',
												type : "POST",
												data : {
													"cardA" : $("#cardA").find(
															"#cardValue").val(),
